...
- `admin/js/media-enhance-bootstrap.js` - New. Observes media modal, injects button, lazy-loads React bundle.
- `admin/css/photobooster-ai-admin.css` - Add minimal overlay/button styles if needed.
- `admin/react-app/` - New. Vite + React + TypeScript source for the app shell.
  ...
  - `admin/react-app/src/App.tsx` - Minimal shell component.
  - `admin/react-app/.gitignore` - Ignore node_modules, dist, and other local files.
- `admin/dist/` - New. Built assets output (JS/CSS) and `manifest.json` for URL resolution.
- `includes/class-photobooster-ai-rest.php` - New. REST route scaffold under plugin namespace (no-op in v1).
...
- [ ] 1.0 Scaffold Vite + React + TypeScript app under `admin/react-app/` and outputs to `admin/dist/`
  ...
  - [ ] 1.5 Verify build outputs hashed assets and `manifest.json` into `admin/dist/`.
  - [x] 1.6 Add `.gitignore` file with all the non-versioned files(like `node_modules`).

- [ ] 2.0 Add PHP helpers to resolve Vite manifest assets and register admin handles
...
- [ ] 3.0 Implement media modal bootstrap to inject “✨ AI Enhance” button into `.attachment-info`
  ...
  - [ ] 3.3 On selection change, if the selected attachment MIME is `image/jpeg`, `image/png`, or `image/webp`, show the button.
  - [x] 3.4 Inject the button at the top of `.attachment-info`; dedupe with a marker attribute (e.g., `data-pbai-enhance="1"]).
  - [ ] 3.5 Hide/remove the button for ineligible selections and when the modal closes.

- [ ] 4.0 Implement lazy-loading to import built React bundle and mount overlay container
  ...
  - [ ] 4.3 Mount the React app and pass attachment props (ID, preview URL) to the shell.
  - [x] 4.4 Implement cleanup: unmount React, remove container, restore focus to the trigger button, handle repeated opens.

- [ ] 5.0 Build React app shell with attachment preview, placeholder controls, and close behavior
  - [ ] 5.1 Implement overlay layout (header with title and Close, content with preview area).
  - [ ] 5.2 Display selected attachment preview (thumbnail/full if available); handle missing preview gracefully.
  - [ ] 5.3 Implement Close button and onClose callback to unmount/cleanup.
  - [ ] 5.4 Add ESC key handler and a basic focus trap within the overlay.

- [ ] 6.0 Add security scaffolding: capability gating (`upload_files`), nonce plumbing, REST route no-op
  ...
  - [ ] 6.3 Use a permission callback that requires `upload_files` and verifies the nonce.
  - [ ] 6.4 Prepare utility functions for future write operations (validate attachment ownership, file type checks).

- [ ] 7.0 Add minimal CSS and ensure overlay accessibility basics (Esc close, focus)
  - [ ] 7.1 Style the injected button to match WP admin buttons; ensure it doesn’t shift layout.
  - [ ] 7.2 Style the overlay container (z-index, backdrop, responsive sizing within the modal).
  - [ ] 7.3 Set `role="dialog"`, `aria-modal="true"`, and label the dialog for screen readers; verify keyboard navigation.
