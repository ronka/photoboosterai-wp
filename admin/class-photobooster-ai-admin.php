...
class Photobooster_Ai_Admin
{
	...
	const LOCALIZE_OBJECT_NAME = 'photobooster_ai_enhance';
	...
	/**
	 * Build the product enhancement inline JavaScript string.
	 *
	 * Returns the JS code to be enqueued via wp_add_inline_script().
	 *
	 * @since 1.0.0
	...
	private function render_product_enhancement_script($attachment_data)
	{
		...
		$id_js     = intval($attachment_id);
		$title_js  = wp_json_encode($attachment->post_title);
		$fname_js  = wp_json_encode($attachment_data['filename']);
		$url_js    = wp_json_encode($image_url);
		$mime_js   = wp_json_encode($attachment->post_mime_type);
		$alt_js    = wp_json_encode($attachment_data['alt_text']);
		$url_large = wp_json_encode(wp_get_attachment_image_url($attachment_id, 'large'));
		$url_med   = wp_json_encode(wp_get_attachment_image_url($attachment_id, 'medium'));
		$url_thumb = wp_json_encode(wp_get_attachment_image_url($attachment_id, 'thumbnail'));
		...
		$js = 'jQuery(document).ready(function($) {
			if ($("#postimagediv .inside").length && !$("#pbai-product-enhance-btn").length) {
				...
				enhanceBtn.on("click", function(e) {
					...
					var pbaiData = window.photobooster_ai_enhance || {};
					var entry = pbaiData.manifest && pbaiData.manifest["src/mount.tsx"];
					var distUrl = pbaiData.distUrl || "";
					...
					var manifest = pbaiData.manifest || {};
					...
					if (typeof window.mountAppIntoModal === "function") {
						...
						import(jsUrl).then(function(mod) {
							...
							if (typeof mountAppFn === "function") {
								...
								mountNode.addEventListener("pbai:close", function() { modalContainer.remove(); });
								mountAppFn(mountNode, { attachment: attachment });
							} else {
		...
		wp_add_inline_script($this->get_bootstrap_script_handle(), $js);
	}
